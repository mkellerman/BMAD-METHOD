...
<parameter name="You MUST have already loaded and processed: {bmad_folder}/bmb/workflows/convert-legacy/workflow.yaml</critical>
...
<action>Show user the target location: {bmad_folder}/{{target_module}}/{{item_type}}/{{item_name}}</action>
<action>Note: Files will be created in {bmad_folder}/ but all internal paths will reference {bmad_folder}/ locations</action>
...
5. Handle dependencies and paths:
   ...
   - CRITICAL: All paths must use {bmad_folder}/{{module}}/ NOT src/

<action>Generate the converted v6 agent YAML file (.agent.yaml)</action>
<action>Example path conversions:

- exec="{bmad_folder}/{{target_module}}/tasks/task-name.md"
- run-workflow="{bmad_folder}/{{target_module}}/workflows/workflow-name/workflow.yaml"
- data="{bmad_folder}/{{target_module}}/data/data-file.yaml"
  </action>
...
<invoke-workflow>
  workflow: {bmad_folder}/bmb/workflows/create-agent/workflow.yaml
  inputs:
...
2. Convert template sections to instructions.md:
   ...
   - `elicit: true` → `<invoke-task halt="true">{bmad_folder}/core/tasks/adv-elicit.xml</invoke-task>` tag
...
config_source: '{bmad_folder}/{{target_module}}/config.yaml'
...
<invoke-workflow>
  workflow: {bmad_folder}/bmb/workflows/create-workflow/workflow.yaml
  inputs:
...
<invoke-workflow>
  workflow: {bmad_folder}/bmb/workflows/create-module/workflow.yaml
  inputs:
...
4. Handle special v4 patterns:
   - 1-9 elicitation menus → v6 <invoke-task halt="true">{bmad_folder}/core/tasks/adv-elicit.xml</invoke-task>
   - Agent permissions → note in instructions
